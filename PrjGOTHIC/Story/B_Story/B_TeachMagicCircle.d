...
func int B_TeachMagicCircle(var C_Npc slf,var C_Npc oth,var int circle)
{
	...
	if(circle == 5)
	{
		...
		B_LogEntry(TOPIC_TalentMagicCircle,"Заклинания 5-го круга: большая огненная буря, ледяная волна, вызов демона, лечение тяжелых ранений и уменьшение монстра.");
		return TRUE;
	};
	if(circle == 6)
	{
		PrintScreen(PRINT_LearnCircle_6,-1,-1,FONT_Screen,2);
		B_LogEntry(TOPIC_TalentMagicCircle,"Заклинания 6-го круга: огненный дождь, дыхание смерти, волна смерти и армия Тьмы.");
		return TRUE;
	};
	return FALSE;
};
