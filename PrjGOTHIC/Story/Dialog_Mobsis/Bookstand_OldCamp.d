...
func void Bookstand_Milten_03_S1()
{
	if(C_NpcIsHero(self))
	{
		...
		Doc_PrintLine(nDocID,0,"");
		Doc_PrintLines(nDocID,0,"Он думает, что в обрушении шахты виноват этот новый парень. Этот человек действительно необычен. Но лучше ему больше здесь не появляться.");
		Doc_PrintLine(nDocID,0,"");
		Doc_PrintLine(nDocID,0,"Корристо");
		Doc_SetMargins(nDocID,-1,30,20,275,20,1);
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLines(nDocID,1,"Гомез готов со злости разнести все вокруг, и я догадываюсь, что он планирует. Мы должны предупредить магов Воды, пока еще не слишком поздно.");
		Doc_PrintLine(nDocID,1,"");
