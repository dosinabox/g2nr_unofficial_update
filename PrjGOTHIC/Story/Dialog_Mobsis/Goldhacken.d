	...
	if(B_GoldMob_Bestimmung())
	{
		...
		if(CurrentChance <= Hero_HackChance)
		{
			if((GoldCounter >= 20) && (MultiNugget >= 8))
			{
				...
				PrintScreen("ƒобыто 3 золотых самородка!",-1,-1,FONT_ScreenSmall,1);
				Truemmer_Count = 0;
			}
			else if((GoldCounter >= 7) && (MultiNugget >= 5))
			{
				CreateInvItems(hero,ItMi_GoldNugget_Addon,2);
				PrintScreen("ƒобыто 2 золотых самородка!",-1,-1,FONT_ScreenSmall,1);
				Truemmer_Count = 0;
				GoldCounter += 1;
			}
			else
			{
				...
				PrintScreen("ƒобыт 1 золотой самородок!",-1,-1,FONT_ScreenSmall,1);
				Truemmer_Count = 0;
				GoldCounter += 1;
			};
		}
		...
		{
			PrintScreen("«олотые осколки разлетаютс€ во все стороны...",-1,53,FONT_ScreenSmall,1);
			Truemmer_Count += 1;
		};
	}
	else
	{
		PrintScreen("«десь больше нечего делать.",-1,53,FONT_ScreenSmall,2);
		b_endproductiondialog();
	};
};
...
func void PC_Goldhacken_Addon_TSchlag_Info()
{
	...
	if(TruemmerChance < 5)
	{
		PrintScreen("Ќичего...",-1,53,FONT_ScreenSmall,1);
	}
	else
	{
